...
namespace zeno
{
    ...
    namespace zfx
    {
        ...
        zeno::ZfxVariable getAttrValue(const std::string& attrname, ZfxContext* pContext, char channel) {
            ...
            auto spGeom = dynamic_cast<zeno::GeometryObject*>(pContext->spObject.get());
            if (!spGeom)
            ...
            case ATTR_INT: {
                ZfxVariable res;
                res.value = spGeom->get_attrs<int>(pContext->runover, attr_name);
                res.bAttr = true;
                return res;
            }
            case ATTR_FLOAT: {
                ZfxVariable res;
                res.value = spGeom->get_attrs<float>(pContext->runover, attr_name);
                res.bAttr = true;
                return res;
            }
            case ATTR_STRING: {
                ZfxVariable res;
                res.value = spGeom->get_attrs<std::string>(pContext->runover, attr_name);
                res.bAttr = true;
                return res;
            }
            case ATTR_VEC2: {
                ZfxVariable res;
                res.value = spGeom->get_attrs<glm::vec2>(pContext->runover, attr_name);
                res.bAttr = true;
                return res;
            }
            case ATTR_VEC3: {
                if (channel == 'x') {
                    ZfxVariable res;
                    res.value = spGeom->get_attrs<float, 'x'>(pContext->runover, attr_name);
                    res.bAttr = true;
                    return res;
                }
                else if (channel == 'y') {
                    ZfxVariable res;
                    res.value = spGeom->get_attrs<float, 'y'>(pContext->runover, attr_name);
                    res.bAttr = true;
                    return res;
                }
                else if (channel == 'z') {
                    ZfxVariable res;
                    res.value = spGeom->get_attrs<float, 'z'>(pContext->runover, attr_name);
                    res.bAttr = true;
                    return res;
                }
                else {
                    assert(channel == 0);
                    ZfxVariable res;
                    res.value = spGeom->get_attrs<glm::vec3>(pContext->runover, attr_name);
                    res.bAttr = true;
                    return res;
                }
            }
            case ATTR_VEC4: {
                ZfxVariable res;
                res.value = spGeom->get_attrs<glm::vec4>(pContext->runover, attr_name);
                res.bAttr = true;
                return res;
            }
            default:
