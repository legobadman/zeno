...
static bool key_in_any(const AlembicUserData* self, const char* key) {
    std::string k(key);
...
static void collect_keys(const AlembicUserData* self, std::set<std::string>& out) {
...
IUserData2* AlembicUserData::clone() {
    return new AlembicUserData(*this);
}

void AlembicUserData::copy(IUserData2* pUserData) {
    auto* other = static_cast<AlembicUserData*>(pUserData);
    if (other) {
...
bool AlembicUserData::has(const char* key) {
    return key_in_any(this, key);
}

size_t AlembicUserData::size() const {
    std::set<std::string> keys;
    collect_keys(this, keys);
    return keys.size();
}

size_t AlembicUserData::get_string(const char* key, const char* defl, char* ret_buf, size_t cap) const {
    std::string k(key);
    std::string s;
...
void AlembicUserData::set_string(const char* key, const char* sval) {
    m_strings[key] = sval ? sval : "";
}

bool AlembicUserData::has_string(const char* key) const {
    return m_strings.count(key) != 0;
}

int AlembicUserData::get_int(const char* key, int defl) const {
...
void AlembicUserData::set_int(const char* key, int iVal) {
    m_ints[key] = iVal;
}

bool AlembicUserData::has_int(const char* key) const {
    return m_ints.count(key) != 0 || m_floats.count(key) != 0;
}

float AlembicUserData::get_float(const char* key, float defl) const {
...
void AlembicUserData::set_float(const char* key, float fVal) {
    m_floats[key] = fVal;
}

bool AlembicUserData::has_float(const char* key) const {
    return m_floats.count(key) != 0 || m_ints.count(key) != 0;
}

bool AlembicUserData::get_bool(const char* key, bool defl) const {
    auto it = m_bools.find(key);
    if (it != m_bools.end()) return it->second;
    return defl;
}

void AlembicUserData::set_bool(const char* key, bool val) {
    m_bools[key] = val;
}

bool AlembicUserData::has_bool(const char* key) const {
    return m_bools.count(key) != 0;
}

Vec2f AlembicUserData::get_vec2f(const char* key, Vec2f defl) const {
    auto it = m_vec2f.find(key);
    if (it != m_vec2f.end()) return it->second;
    return defl;
}

Vec2i AlembicUserData::get_vec2i(const char* key) const {
    auto it = m_vec2i.find(key);
    if (it != m_vec2i.end()) return it->second;
    return Vec2i(0, 0);
}

void AlembicUserData::set_vec2f(const char* key, const Vec2f& vec) {
    m_vec2f[key] = vec;
}

void AlembicUserData::set_vec2i(const char* key, const Vec2i& vec) {
    m_vec2i[key] = vec;
}

bool AlembicUserData::has_vec2f(const char* key) const {
    return m_vec2f.count(key) != 0;
}

bool AlembicUserData::has_vec2i(const char* key) const {
    return m_vec2i.count(key) != 0;
}

Vec3f AlembicUserData::get_vec3f(const char* key, Vec3f defl) const {
    auto it = m_vec3f.find(key);
    if (it != m_vec3f.end()) return it->second;
    return defl;
}

Vec3i AlembicUserData::get_vec3i(const char* key) const {
    auto it = m_vec3i.find(key);
    if (it != m_vec3i.end()) return it->second;
    return Vec3i(0, 0, 0);
}

void AlembicUserData::set_vec3f(const char* key, const Vec3f& vec) {
    m_vec3f[key] = vec;
}

void AlembicUserData::set_vec3i(const char* key, const Vec3i& vec) {
    m_vec3i[key] = vec;
}

bool AlembicUserData::has_vec3f(const char* key) const {
    return m_vec3f.count(key) != 0;
}

bool AlembicUserData::has_vec3i(const char* key) const {
    return m_vec3i.count(key) != 0;
}

Vec4f AlembicUserData::get_vec4f(const char* key) const {
    auto it = m_vec4f.find(key);
    if (it != m_vec4f.end()) return it->second;
    return Vec4f{0.f, 0.f, 0.f, 0.f};
}

Vec4i AlembicUserData::get_vec4i(const char* key) const {
    auto it = m_vec4i.find(key);
    if (it != m_vec4i.end()) return it->second;
    return Vec4i{0, 0, 0, 0};
}

void AlembicUserData::set_vec4f(const char* key, const Vec4f& vec) {
    m_vec4f[key] = vec;
}

void AlembicUserData::set_vec4i(const char* key, const Vec4i& vec) {
    m_vec4i[key] = vec;
}

bool AlembicUserData::has_vec4f(const char* key) const {
    return m_vec4f.count(key) != 0;
}

bool AlembicUserData::has_vec4i(const char* key) const {
    return m_vec4i.count(key) != 0;
}

size_t AlembicUserData::get_float_arr(const char* key, float* buf, size_t cap) const {
...
void AlembicUserData::del(const char* key) {
    std::string k(key);
    m_strings.erase(k);
    m_ints.erase(k);
    m_floats.erase(k);
