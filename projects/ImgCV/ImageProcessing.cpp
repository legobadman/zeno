...
struct ImageBlur : INode {
    virtual void apply() override {
        ...
        int w = ud->get_int("w");
        int h = ud->get_int("h");
        auto img_out = create_GeometryObject(true, w * h, 0);
...
struct ImageToNormalMap : INode {
    virtual void apply() override {
        ...
        auto normalmap = create_GeometryObject(true, w * h, 0);
...
struct ImageTile: INode {
    void apply() override {
        ...
        int w1 = w * cols;
        int h1 = h * rows;
        auto image2 = create_GeometryObject(true, w1 * h1, 0);
...
#pragma omp parallel for
    for (int iter = 0; iter < iterations; iter++){
        auto imagetmp = create_GeometryObject(true, image_width * image_height, 0);
...
struct ImageColor : INode {
    virtual void apply() override {
        ...
        auto image = create_GeometryObject(true, vertsize, 0);
